...
def AIMakeMove(img):
    ...
    boardDetected = chessboard.detectGameState(img)
...
while True:
    boardDetected = False
    while boardDetected is False:
        img = helpers.captureBoard()
        print("AI's turn...")
        boardDetected = AIMakeMove(img)
    print("AI finished it's turn.")
    humanFinishedMove = False
    handInsideBoard = False
    print("Human's turn...")
    while True:
        handDetected = helpers.detectHand(helpers.captureBoard())
        print("HandDetected: ", handDetected)
        if (handInsideBoard is True) and (handDetected is False):
            print("Human finished it's turn.")
            break
        else:
            handInsideBoard = handDetected
